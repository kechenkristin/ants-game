...
def choose_bee(bees):
    ...
    assert isinstance(
        bees, list), "choose_bee's argument should be a list but was a %s" % type(bees).__name__
    if bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        curr_place = self.place
        all_bees = curr_place.bees
        all_bees_copy = all_bees[:]
        ...
        for bee in all_bees_copy:
            Insect.reduce_health(bee, total_damage)

        # make the fireant take damage from the bee
...
class HungryAnt(Ant):
    ...
    After eating, HungryAnt must spend 3 turns chewing before it eats again."""
    ...
    def __init__(self, health=1):
        """Create a HungryAnt with Health quantity and chew_countdown(initialize to 0) quantity."""
    ...
    def action(self, gamestate):
        ...
        if self.chew_countdown != 0:
            ...
            eaten = self.eat(self.place.bees)
            if eaten:
                self.chew_countdown = HungryAnt.chew_duration

    def eat(self, bees):
        random_bee = choose_bee(bees) if bees != [] else None
        if random_bee == None:
            return False
        else:
            Insect.reduce_health(random_bee, random_bee.health)
            return True
# END Problem 7
...
class TankAnt(ContainerAnt):
    """
    ThankAnt is a container that protects an ant in its place and also deals
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        if self.direction == False and not self.place.entrance.is_hive:
